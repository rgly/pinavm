...
void
SimpleWriter::printStructReturnPointerFunctionType(formatted_raw_ostream &Out,
...
raw_ostream &
SimpleWriter::printSimpleType(formatted_raw_ostream &Out, const Type *Ty,
			      bool isSigned,
...
std::ostream &
SimpleWriter::printSimpleType(std::ostream &Out, const Type *Ty, bool isSigned,
			      const std::string &NameSoFar)
{
  assert((Ty->isPrimitiveType() || Ty->isInteger() || isa<VectorType>(Ty)) && 
         "Invalid type for printSimpleType");
			...
			bool IgnoreName, const AttrListPtr &PAL)
{
  ...
  case Type::StructTyID: {
    ...
    DEBUG_MSG("/**** Handling StructTy type in printType() ****/\n");
//     Out << NameSoFar + " {\n";
    unsigned Idx = 0;
...
//       Out << " __attribute__ ((packed))";
  ...
  case Type::PointerTyID: {
    ...
    DEBUG_MSG("/**** Handling PointerTy type in printType() ****/\n");
...
/// @brief Print a cast
void
SimpleWriter::printCast(unsigned opc, const Type *SrcTy, const Type *DstTy)
{ 
...
SimpleWriter::printConstant(Constant *CPV, bool Static)
{
  ...
  if (const ConstantExpr *CE = dyn_cast<ConstantExpr>(CPV)) {
    ...
    default:
#ifndef NDEBUG
      errs() << "SimpleWriter Error: Unhandled constant expression: "
	     << *CE << "\n";
#endif
      llvm_unreachable(0);
    }
...
bool
SimpleWriter::printConstExprCast(const ConstantExpr* CE, bool Static)
{
...
void
SimpleWriter::printConstantWithCast(Constant* CPV, unsigned Opcode)
{
...
std::string
SimpleWriter::GetValueName(const Value *Operand)
{
...
SimpleWriter::writeInstComputationInline(Instruction &I)
{
  ...
  if (Ty->isInteger() && (Ty!=Type::getInt1Ty(I.getContext()) &&
    ...
    llvm_report_error("The Simple backend does not currently support integer "
  ...
  DEBUG_MSG("\n /***** Visiting " << I.getOpcodeName() << " ( writeInstComputationInline() ) ******/ \n");
  visit(I);
...
bool
SimpleWriter::isAddressExposed(const Value *V) const
{
...
// for the Instruction.
bool
SimpleWriter::writeInstructionCast(const Instruction &I)
{
...
void
SimpleWriter::writeOperandWithCast(Value* Operand, unsigned Opcode)
{
...
// being used. 
void
SimpleWriter::writeOperandWithCast(Value* Operand, const ICmpInst &Cmp)
{
...
///
void
SimpleWriter::printModuleTypes(const TypeSymbolTable &TST)
{
...
//
void
SimpleWriter::printContainedStructs(const Type *Ty,
                                    std::set<const Type*> &StructPrinted)
{
...
// 	  DEBUG_MSG("\n/**** before outputLValue ****/\n");
  ...
  DEBUG_MSG("/***** visit terminator : " << *BB->getTerminator()->getOpcodeName() << "*****/\n");
...
SimpleWriter::visitReturnInst(ReturnInst &I)
{
  ...
  DEBUG_MSG("/**** Nb operands : " << I.getNumOperands() << "****/\n");
...
bool
SimpleWriter::isGotoCodeNecessary(BasicBlock *From, BasicBlock *To)
{
...
void
SimpleWriter::printPHICopiesForSuccessor (BasicBlock *CurBlock,
...
void
SimpleWriter::printBranchToBlock(BasicBlock *CurBB, BasicBlock *Succ,
                                 unsigned Indent)
{
...
bool
SimpleWriter::visitBuiltinCall(CallInst &I, Intrinsic::ID ID,
                               bool &WroteCallee)
{
  switch (ID) {
  default: {
...
void
SimpleWriter::visitMallocInst(MallocInst &I)
{
...
void
SimpleWriter::visitFreeInst(FreeInst &I)
{
  llvm_unreachable("lowerallocations pass didn't work!");
}

void
SimpleWriter::printGEPExpression(Value *Ptr, gep_type_iterator I,
...
void
SimpleWriter::writeMemoryAccess(Value *Operand, const Type *OperandType,
...
const char *
SimpleWriter::getPassName() const
{
