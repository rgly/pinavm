...
FunctionBuilder::FunctionBuilder(Process* process, Function* origFunction, Function* functionToJit, Value* resValue)
{
  this->proc = process;
...
bool
FunctionBuilder::mark(Value* arg)
{
...
//     }
  ...
  if (isa<Constant>(arg)  || isa<GlobalValue>(arg) || isa<BitCastInst>(arg)) {
    TRACE_6("** Arg is an arg or constant or global value or bitcast\n");
    return false;
  }
