...
INSTALL_PATH_SYSTEMC=$HOME/local/test-llvm/lib/systemc-2.2.0-llvm

# ##### INITIALIZATION #####
 test -d "$DOWNLOAD_AND_COMPILE_DIR" || \
     (echo "$DOWNLOAD_AND_COMPILE_DIR does not exist, creating it" && mkdir -p "$DOWNLOAD_AND_COMPILE_DIR")

test -d "$INSTALL_PATH_LLVM" || \
    (echo "$INSTALL_PATH_LLVM does not exist, creating it" && mkdir -p "$INSTALL_PATH_LLVM")

test -d "$INSTALL_PATH_SYSTEMC" || \
    (echo "$INSTALL_PATH_SYSTEMC does not exist, creating it" && mkdir -p "$INSTALL_PATH_SYSTEMC")
...
do
    ...
    test -d "$INSTALL_PATH_SYSTEMC/include/sysc/$DIRNAME_CORRECTED" || \
	mkdir -p "$INSTALL_PATH_SYSTEMC/include/sysc/$DIRNAME_CORRECTED"
